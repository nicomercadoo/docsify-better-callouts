...
function applyTagOverrides(config, overrides, defaultTag2) {
  if (!overrides)
    return config;
  for (const [userTag, userTagConfig] of Object.entries(overrides)) {
    ...
    if (!matched) {
      handleNewTag(config, defaultTag2, userTag, userTagConfig);
    }
  }
  return config;
}
...
function handleNewTag(config, defaultTag2, userTag, userTagConfig) {
